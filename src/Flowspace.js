          ...
          if (outputs instanceof Array) {
            ...
            Object.keys(outputs).map((out_key) => {
              ...
              connections.push({
                ...
                inputColor:
                  output.inputColor ||
                  (this.props.noFade ? theme_colors.p : theme_colors.a),
                arrowStart: output.arrowStart,
                arrowEnd: output.arrowEnd,
                dash:
                  output.dash !== undefined
                    ? output.dash > 0
                      ? output.dash
                      : undefined
                    : undefined,
                onClick: output.onClick
                  ...
                  ? (e) => {
                      this.props.onLineClick(child.key, out_key, e);
                    }
                  : null,
              });
            });
      ...
      connections.map((connection, index) => {
        ...
        if (pa && pb) {
          ...
          var positions = AutoGetLoc(
            ...
            connection.b,
            this.state,
            this.props.avoidCollisions === false ? false : true
          );

          // Display arrows - if set then connection-specific overrides flowspace
          var markerStart = this.props.arrowStart;
          if (connection.arrowStart !== undefined)
            markerStart = connection.arrowStart;
          var markerEnd = this.props.arrowEnd;
          if (connection.arrowEnd !== undefined)
            markerEnd = connection.arrowEnd;

          // Adding coloured arrow-marker definitions to list (if not already present)
          if (markerStart && !defs[connection.outputColor])
          ...
          if (markerEnd && !defs[connection.inputColor])
          ...
          if (this.props.selectedLine) {
            if (
              this.props.selectedLine.a === connection.a &&
              this.props.selectedLine.b === connection.b
            )
              isSelectedLine = true;
          }
          paths.push(
            <path
              ...
              strokeWidth={
                parseInt(connection.width) + (isSelectedLine ? 3 : 0)
              }
              onClick={connection.onClick}
              markerStart={
                markerStart ? "url(#arrow" + connection.outputColor + ")" : null
              }
              markerEnd={
                markerEnd ? "url(#arrow" + connection.inputColor + ")" : null
              }
            />
          );
    ...
    return (
      ...
      >
        ...
        >
          <div
            ref={(ref) => {
              if (this.props.getDiagramRef) this.props.getDiagramRef(ref);
            }}
            style={{
              width: "100%",
              height: "100%",
